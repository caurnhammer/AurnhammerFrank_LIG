...
# Trains a series of language models                                          #
###############################################################################
...
def train(seed_index, t_plus_one):
    # chr: These trianing function is manipulated to present the word after the next word as the correct target
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        for j in range(len(data)):
            if (data[j].data == a)[0] == True: # = end of sentence is reached - remove filler masking elements
                data = data[:j,:1]
                if t_plus_one:
                    # Chr: here the targets are manipulated by setting them one step ahead
                    targets = targets[1:j+1]
                else:
                    targets = targets[:j]
                break
...
for seed_index, seed in zip(seed_indices, seeds):
    ...
    models = ['LSTM'] # chr: We only used LSTMs for this paper
    for model_name in models:
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                # Set t_plus_one to False to predict the next word instead of the word after the next word
                train(seed_index, t_plus_one=True)
